...
import { filterRows, rangeDays, resolveRange, totalsByDay, totalsByModel } from '../src/ranges.ts'
...
test('filterRows：范围外过滤、all 全量（7d/month 经 resolveRange）', () => {
  const rows = [row('2026-08-01'), row('2026-08-24'), row('2026-08-25'), row('2026-09-01')]
  const range7 = resolveRange('7d', TODAY)
  const rangeMonth = resolveRange('month', TODAY)
  assert.equal(filterRows(rows, { range: range7 }).length, 2)
  assert.equal(filterRows(rows, { range: rangeMonth }).length, 3)
  assert.equal(filterRows(rows, { range: null }).length, 4)
})
