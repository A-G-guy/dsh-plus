    ...
    JSON.stringify({
      ...
      data: {
        'acme-lab': {
          id: 'acme-lab',
          models: {
            'acme-huge': { id: 'acme-huge', name: 'Acme Huge', reasoning: true, limit: { context: 512000, output: 64000 } },
          },
        },
      },
    }),
...
test('parseExtendsRef 解析两种形态', () => {
  assert.deepEqual(parseExtendsRef('deepseek/deepseek-v4-flash'), { provider: 'deepseek', model: 'deepseek-v4-flash' })
  assert.deepEqual(parseExtendsRef('deepseek-v4-flash'), { model: 'deepseek-v4-flash' })
...
test('显式 "provider/model" 引用命中内置目录', () => {
  const hit = resolveModelBase('chat', {}, { id: 'flash', extends: 'deepseek/deepseek-v4-flash' }, kit, undefined)
...
test('裸 model id 随 route 级 extends 源查找', () => {
  const hit = resolveModelBase('chat', { extends: 'deepseek' }, { id: 'x', extends: 'deepseek-v4-pro' }, kit, undefined)
  assert.equal(hit.source, 'builtin')
...
test('内置未收录时回退 models.dev 快照（字段保守）', async () => {
  ...
  const hit = resolveModelBase('chat', {}, { id: 'h', extends: 'acme-lab/acme-huge' }, kit, modelsDev)
...
test('缺省 extends 且 route 有 extends 源时按同名模型继承；无 extends 源时为手写模型', () => {
  const hit = resolveModelBase('chat', { extends: 'deepseek' }, { id: 'deepseek-v4-flash' }, kit, undefined)
  assert.equal(hit.source, 'builtin')
